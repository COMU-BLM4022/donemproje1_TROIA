...
monthly_services = df.groupby("NEW_DATE").size()

# Aylık hizmet sayısını dataframe olarak düzenleme
monthly_services_df = monthly_services.to_frame(name="Hizmet Sayısı").reset_index()

# Zaman serisi grafiği oluşturma
plt.figure(figsize=(12, 6))
plt.plot(monthly_services_df["NEW_DATE"].astype(str), monthly_services_df["Hizmet Sayısı"], marker='o')
plt.title("Aylık Hizmet Sayısı Zaman Serisi")
plt.xlabel("Tarih")
plt.ylabel("Hizmet Sayısı")
